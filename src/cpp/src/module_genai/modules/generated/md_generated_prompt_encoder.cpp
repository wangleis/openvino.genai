...
#include "md_generated_prompt_encoder.hpp"
...
GENAI_REGISTER_MODULE_SAME(GeneratedPromptEncoderModule);
...
void GeneratedPromptEncoderModule::print_static_config() {
    std::cout << R"(
  prompt_encoder:                       # Module Name
    type: "GeneratedPromptEncoderModule"
...
GeneratedPromptEncoderModule::GeneratedPromptEncoderModule(const IBaseModuleDesc::PTR& desc, const PipelineDesc::PTR& pipeline_desc)
    : IBaseModule(desc, pipeline_desc) {
    if (!initialize()) {
        GENAI_ERR("Failed to initiate GeneratedPromptEncoderModule");
    }
}

GeneratedPromptEncoderModule::~GeneratedPromptEncoderModule() {}

bool GeneratedPromptEncoderModule::initialize() {
    const auto& params = module_desc->params;
    auto it_path = params.find("model_path");
    if (it_path == params.end()) {
        std::cerr << "GeneratedPromptEncoderModule[" << module_desc->name << "]: 'model_path' not found in params" << std::endl;
        return false;
    }
...
void GeneratedPromptEncoderModule::run() {
    ...
    if (exists_input("prompts")) {
        ...
        OPENVINO_ASSERT(false, "GeneratedPromptEncoderModule[" + module_desc->name + "]: No prompt input found.");
    }
    ...
    if (has_encoded_image) {
        ...
        if (model_type != VLMModelType::QWEN2_VL && model_type != VLMModelType::QWEN2_5_VL) {
            GENAI_ERR("GeneratedPromptEncoderModule[" + module_desc->name + "]: Unsupported model type: " + module_desc->model_type);
            return;
        }
    }
...
std::pair<TokenizedInputs, std::vector<int>> GeneratedPromptEncoderModule::run(const std::vector<std::string>& prompts, 
                        const std::vector<ov::Tensor>& encoded_images,
                        const std::vector<std::vector<int>>& source_sizes,
                        bool has_encoded_image) {
    OPENVINO_ASSERT(m_tokenizer_impl, "GeneratedPromptEncoderModule is not initialized. Call initialize() first.");
...
NormalizedPrompt GeneratedPromptEncoderModule::normalize_prompt(const std::string& prompt,
...
std::pair<std::string, std::vector<size_t>> GeneratedPromptEncoderModule::normalize(
...
size_t GeneratedPromptEncoderModule::calc_tokens_num(size_t grid_t, size_t grid_h, size_t grid_w) const {
