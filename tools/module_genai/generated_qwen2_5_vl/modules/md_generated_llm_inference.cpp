...
bool GeneratedLlmInferenceModule::load_generation_config(const std::filesystem::path& config_path) {
    try {
...
bool GeneratedLlmInferenceModule::initialize() {
    ...
    try {
    	...
    	GENAI_ERR("GeneratedLlmInferenceModule[" + module_desc->name + "]: Failed to create pipeline: " + e.what());
        return false;
    }
}
...
void GeneratedLlmInferenceModule::run() {
    ...
    if (this->inputs.find("embeds") != this->inputs.end()) {
        ...
        GENAI_ERR("GeneratedLlmInferenceModule[" + module_desc->name + "]: 'embeds or embeds_list' input not found")
    }
    ...
    GENAI_INFO("GeneratedLlmInferenceModule[" + module_desc->name + "] generation completed.");
}
