...
GeneratedVisionEncoderModule::GeneratedVisionEncoderModule(const IBaseModuleDesc::PTR& desc, const PipelineDesc::PTR& pipeline_desc)
    ...
    if (model_type != VLMModelType::QWEN2_VL && model_type != VLMModelType::QWEN2_5_VL) {
        GENAI_ERR("GeneratedVisionEncoderModule[" + desc->name + "]: Unsupported model type: " + desc->model_type);
        return;
    }
    if (!initialize()) {
        GENAI_ERR("Failed to initiate GeneratedVisionEncoderModule");
    }
}
...
bool GeneratedVisionEncoderModule::initialize() {
    ...
    if (it_path == params.end()) {
        GENAI_ERR("GeneratedVisionEncoderModule[" + module_desc->name + "]: 'model_path' not found in params");
        return false;
    }
    ...
    if (it_vision_start_token_id != params.end()) {
        ...
        GENAI_ERR("GeneratedVisionEncoderModule[" + module_desc->name + "]: 'vision_start_token_id' not found in params, using default 0");
    }
    ...
    if (!std::filesystem::exists(model_path / "openvino_vision_embeddings_merger_model.xml")) {
        GENAI_ERR("GeneratedVisionEncoderModule[" + module_desc->name + "]: model file not found at " + 
...
void GeneratedVisionEncoderModule::run() {
    ...
    if (this->inputs.find("preprocessed_image") == this->inputs.end() || this->inputs["preprocessed_image"].data == nullptr) {
        GENAI_ERR("GeneratedVisionEncoderModule[" + module_desc->name + "]: 'preprocessed_image' input not found");
        return;
    }
    if (this->inputs.find("source_size") == this->inputs.end() || this->inputs["source_size"].data.as<std::vector<int>>().empty()) {
        GENAI_ERR("GeneratedVisionEncoderModule[" + module_desc->name + "]: 'source_size' input not found");
        return;
    }
    if (this->inputs.find("images_sequence") == this->inputs.end() || this->inputs["images_sequence"].data.as<std::vector<int>>().empty()) {
        GENAI_ERR("GeneratedVisionEncoderModule[" + module_desc->name + "]: 'images_sequence' input not found");
        return;
    }

    ov::Tensor input_ids;
    if (this->inputs.find("input_ids") == this->inputs.end() || this->inputs["input_ids"].data == nullptr) {
        GENAI_WARN("GeneratedVisionEncoderModule[" + module_desc->name +
...
std::pair<ov::Tensor, ov::Tensor> GeneratedVisionEncoderModule::embed(const EncodedImage &image, const std::vector<int>& images_sequence, const ov::Tensor& input_ids) {
    OPENVINO_ASSERT(m_ireq_queue_vision_embeddings_merger, "GeneratedVisionEncoderModule is not initialized. Call initialize() first.");
...
ov::Tensor GeneratedVisionEncoderModule::get_rotary_pos_emb(const std::vector<std::array<size_t, 3>>& grids_thw) {
...
size_t GeneratedVisionEncoderModule::calc_vec_tokens_num(const std::vector<std::array<size_t, 3UL>>& vec_grid_thw) const {
    size_t token_num = 0;
...
size_t GeneratedVisionEncoderModule::calc_tokens_num(size_t grid_t, size_t grid_h, size_t grid_w) const {
    return grid_t * grid_h * grid_w / m_merge_length;
}

ov::Tensor GeneratedVisionEncoderModule::create_position_ids(
