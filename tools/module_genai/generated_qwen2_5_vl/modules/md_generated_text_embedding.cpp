...
GeneratedTextEmbeddingModule::GeneratedTextEmbeddingModule(const IBaseModuleDesc::PTR& desc, const PipelineDesc::PTR& pipeline_desc)
    ...
    if (model_type != VLMModelType::QWEN2_VL && model_type != VLMModelType::QWEN2_5_VL) {
        GENAI_ERR("GeneratedTextEmbeddingModule[" + desc->name + "]: Unsupported model type: " + desc->model_type);
    }
    if (!initialize()) {
        GENAI_ERR("Failed to initiate GeneratedTextEmbeddingModule");
    }
}
...
bool GeneratedTextEmbeddingModule::initialize() {
    ...
    if (it_dir == params.end()) {
        GENAI_ERR("GeneratedTextEmbeddingModule[" + module_desc->name + "]: 'model_path' not found in params");
        return false;
    }
    ...
    if (it_scale != params.end()) {
        try {
            ...
            GENAI_ERR("GeneratedTextEmbeddingModule[" + module_desc->name + "]: invalid 'scale_emb' value, fallback to 1.0");
        }
    }
...
void GeneratedTextEmbeddingModule::run() {
    ...
    if (this->inputs.find("input_ids") == this->inputs.end()) {
        GENAI_ERR("GeneratedTextEmbeddingModule[" + module_desc->name + "]: 'input_ids' input not found")
    }
