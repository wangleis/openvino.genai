...
#include "md_generated_embedding_merger.hpp"
...
GENAI_REGISTER_MODULE_SAME(GeneratedEmbeddingMergerModule);

void GeneratedEmbeddingMergerModule::print_static_config() {
    std::cout << R"(
  embedding_merger:
    type: "GeneratedEmbeddingMergerModule"
    device: "GPU"
    inputs:
      - name: "input_ids"
...
GeneratedEmbeddingMergerModule::GeneratedEmbeddingMergerModule(const IBaseModuleDesc::PTR& desc, const PipelineDesc::PTR& pipeline_desc)
    ...
    if (model_type != VLMModelType::QWEN2_VL && model_type != VLMModelType::QWEN2_5_VL) {
        GENAI_ERR("GeneratedEmbeddingMergerModule[" + desc->name + "]: Unsupported model type: " + desc->model_type);
    }
    if (!initialize()) {
        GENAI_ERR("Failed to initiate GeneratedEmbeddingMergerModule");
    }
}

GeneratedEmbeddingMergerModule::~GeneratedEmbeddingMergerModule() {}

bool GeneratedEmbeddingMergerModule::initialize() {
    const auto &params = module_desc->params;
    auto it_path = params.find("model_path");
    if (it_path == params.end()) {
        GENAI_ERR("GeneratedEmbeddingMergerModule[" + module_desc->name + "]: 'model_path' not found in params");
        return false;
    }
...
void GeneratedEmbeddingMergerModule::run() {
    GENAI_INFO("Running module: " + module_desc->name);

    prepare_inputs();
    if (this->inputs.find("input_ids") == this->inputs.end()) {
        GENAI_ERR("GeneratedEmbeddingMergerModule[" + module_desc->name + "]: 'input_ids' input not found");
        return;
    }
    if (this->inputs.find("input_embedding") == this->inputs.end()) {
        GENAI_ERR("GeneratedEmbeddingMergerModule[" + module_desc->name + "]: 'input_embedding' input not found");
        return;
    }
...
void GeneratedEmbeddingMergerModule::encode_vision_placeholder_tokens() {
...
ov::Tensor GeneratedEmbeddingMergerModule::merge_text_and_video_image_embeddings(
